    ...
    public class Task5
    {
        ...
        public struct Research
        {
            ...
            private string[] SortedQuationByPopulalarAnswersAndChooseFivePeopleOrLess(string[] array)
            {
                ...
                // 3. Добавим в конце каждого элемента, число повторов.
                ...
                var sortedArray = copyArrayWithOutNull.OrderByDescending(x => (int)x[^1]).ToArray();

                // 5. Удаляем раннее добавленные цифры повторов
                for (int i = 0; i < sortedArray.Length; i++)
                    sortedArray[i] = sortedArray[i].Substring(0, sortedArray[i].Length - 1);

                // 6. Удаляем повторяющиеся элементы. 
                var sortedArrayWithDeleteRepetitions = sortedArray.Distinct().ToArray();
